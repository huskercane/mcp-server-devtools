...
/// Adapter constructor, invoked with the server's configured audit sink.
pub type GateFactory = fn(
    &Config,
    Option<&Arc<dyn AuditSink>>,
    std::time::Duration,
) -> Result<approvals::Selected, String>;
...
pub struct ServerBuilder {
    ...
    gate_factory: Option<GateFactory>,
}
...
impl ServerBuilder {
    /// Supply an edition's admission adapter without opening a second journal.
    #[must_use]
    pub fn gate_factory(mut self, factory: GateFactory) -> Self {
        self.gate_factory = Some(factory);
        self
    }
...
fn select_gate(
    ...
    let selected = if let Some(gate) = injected {
        ...
        let selected =
            factory.unwrap_or(approvals::gate_from_config)(config, audit_sink, append_timeout)
                .map_err(|error| {
                    crate::error::unexpected(format!("{error}; refusing to start"), None)
                })?;
...
pub fn configured_journal_dir(config: &Config) -> Option<&str> {
    config
