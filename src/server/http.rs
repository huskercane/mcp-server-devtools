...
pub type ControlMount = Box<dyn FnOnce(&Router, &DevtoolsServer) -> Router + Send>;

/// Optional control-plane adapters supplied by the executable.
#[derive(Default)]
pub struct HttpExtensions {
    pub admin: Option<Arc<dyn super::admin::AdminExtension>>,
    pub mount: Option<ControlMount>,
}

/// Startup composition supplied by an edition; no proprietary implementation
/// is linked by the Community executable.
pub trait HttpEdition: Send + Sync {
    fn configure(
        &self,
        builder: crate::bootstrap::ServerBuilder,
    ) -> crate::bootstrap::ServerBuilder {
        builder
    }
    fn setup(
        &self,
    ...
    ) -> Result<HttpExtensions, String>;
}

pub struct CommunityEdition;
impl HttpEdition for CommunityEdition {
    fn setup(
        &self,
        config: &Config,
        _: Role,
        ...
        if config.get("MCP_CONSOLE_CLIENT_ID").is_some() {
            return Err("MCP_CONSOLE_CLIENT_ID requires the Enterprise console".into());
        }
        Ok(HttpExtensions::default())
    }
}
...
pub async fn run_http_as(role: Role) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    run_http_with_edition(role, &CommunityEdition).await
}

/// Boot using explicitly supplied edition adapters.
pub async fn run_http_with_edition(
    role: Role,
    edition: &dyn HttpEdition,
    ...
    let server = edition
        .configure(
            crate::bootstrap::ServerBuilder::new()
                .watch_config(true)
    ...
    let extensions = edition.setup(&config, role, inbound_auth.as_deref(), oidc.as_ref())?;
    if ema {
    ...
    let app = build_app_for_role_with_extensions(
        role,
        server,
        inbound_auth,
        extensions,
...
fn extension_router(mut setup: HttpExtensions, admin: &Router, server: &DevtoolsServer) -> Router {
    setup
        .mount
        .take()
        .map_or_else(Router::new, |mount| mount(admin, server))
}
...
/// control-plane extensions supplied by an embedding executable (see [`Role`]).
pub fn build_app_for_role(
    ...
    build_app_for_role_with_extensions(
        role,
        server,
        auth,
        HttpExtensions::default(),
...
/// Compose optional control-plane extensions on roles that serve control.
/// Administrative operations share the core middleware and task budget;
/// browser mounts implement their own origin/session guards.
pub fn build_app_for_role_with_extensions(
    role: Role,
    server: DevtoolsServer,
    auth: Option<Arc<InboundAuth>>,
    ...
    let admin = auth.as_ref().map_or_else(Router::new, |auth| {
        ...
        super::admin::router_with_extension(
            ...
            super::admin::file_policy_admin(&server),
            server.mutation_gate(),
            extensions.admin.clone(),
        )
    });
    let extra_routes = extension_router(extensions, &admin, &server);
    Router::new()
        .merge(admin)
        .route(
            "/",
...
fn build_app_inner(
    ...
    let admin = if serves_control {
        auth.as_ref().map_or_else(Router::new, |auth| {
            super::admin::router_with_extension(
                ...
                super::admin::file_policy_admin(&shared_server),
                shared_server.mutation_gate(),
                extensions.admin.clone(),
            )
        })
    } else {
    ...
    let extra_routes = if serves_control {
        extension_router(extensions, &admin, &shared_server)
    } else {
        Router::new()
    };
