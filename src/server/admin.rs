...
#[derive(Clone)]
pub struct Admin {
    extension: Option<Arc<dyn AdminExtension>>,
    pub server: DevtoolsServer,
    pub auth: Arc<InboundAuth>,
...
pub fn router_with_policy(
    ...
    router_with_extension(
        server, auth, sessions, artifacts, reports, policy, gate, None,
    )
}

/// Build an admin boundary with extension operations under the same security controls.
#[allow(clippy::too_many_arguments)]
pub fn router_with_extension(
    server: DevtoolsServer,
    auth: &InboundAuth,
    sessions: Option<Arc<dyn SessionStore>>,
    artifacts: Option<Arc<dyn ArtifactStore>>,
...
pub const INVALID: Error = Error(StatusCode::BAD_REQUEST, "invalid_request");
pub const UNAVAILABLE: Error = Error(StatusCode::SERVICE_UNAVAILABLE, "admin_backend_unavailable");
pub const NOT_FOUND: Error = Error(StatusCode::NOT_FOUND, "not_found");
const AUDIT_FAILED: Error = Error(StatusCode::SERVICE_UNAVAILABLE, "audit_unavailable");
pub const fn method_not_allowed() -> Error {
...
impl Admin {
    ...
    async fn execute(
        ...
        if let Some(extension) = &self.extension
            && extension.handles(op)
        {
            return extension
                .execute(self, principal, facts, method, op, body)
                .await;
        }
...
impl Admin {
    ...
    pub async fn apply_candidate(
        &self,
        principal: &Principal,
        candidate: crate::ports::Candidate,
    ) -> Result<u64, Error> {
...
/// Extension operations execute inside the authenticated, bounded admin task.
pub trait AdminExtension: Send + Sync {
    fn handles(&self, operation: &str) -> bool;
    fn execute<'a>(
        &'a self,
        admin: &'a Admin,
        principal: &'a Principal,
        facts: Option<&'a TokenFacts>,
        method: &'a Method,
        operation: &'a str,
        body: &'a [u8],
    ) -> std::pin::Pin<Box<dyn std::future::Future<Output = Result<Outcome, Error>> + Send + 'a>>;
}
