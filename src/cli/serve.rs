...
pub async fn dispatch(opts: ServeOpts) -> ExitCode {
    dispatch_with_edition(opts, &crate::server::http::CommunityEdition).await
}

pub async fn dispatch_with_edition(
    opts: ServeOpts,
    ...
    let result = match transport {
        ...
        TransportArg::Http => {
            ...
            match role {
                Ok(role) => crate::server::http::run_http_with_edition(role, edition).await,
