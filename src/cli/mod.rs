...
{
    run_with_edition(args, &crate::server::http::CommunityEdition).await
}

/// Shared commands with an explicit HTTP composition for `serve`.
pub async fn run_with_edition<I>(
    args: I,
    edition: &dyn crate::server::http::HttpEdition,
) -> ExitCode
where
    I: IntoIterator<Item = String>,
{
    ...
    dispatch_command(cli.command, edition).await
}

/// Dispatch a parsed command using the caller's HTTP composition.
pub async fn dispatch_command(
    ...
    let result = match command {
        ...
        TopCommand::Serve(opts) => return serve::dispatch_with_edition(opts, edition).await,
