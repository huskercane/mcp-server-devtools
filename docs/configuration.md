...
| `MCP_ADMIN_APPROVALS` | `off` | `off` or `required`. Which admission gate every `/admin/*` mutation (policy reload and install, deny-list replacement, session revoke, artifact purge) consults **before** it appends its durable `admin_mutation` intent (plan §3.10.1, `ports::MutationGate`). `off`, or unset, is the direct gate: every mutation is admitted and behaves exactly as it did before the gate existed. `required` names the two-person approval adapter of plan §3.10.3 (WP D.2); until that adapter ships, the server refuses to start with a message naming this key, so a deployment that asks for approvals never runs ungated by accident. Any other value is refused at startup. A held mutation answers `202` and a refused one `409` — the shapes are in [the admin API runbook](admin-api-runbook.md). |
