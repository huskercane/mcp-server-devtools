...
| `MCP_AUDIT_APPEND_TIMEOUT_MS` | `5000`; clamped to `100`–`60000` | Bound on the wait for durable acknowledgement. Before dispatch, a timeout refuses the call. After dispatch, the outcome record is never abandoned: the caller stops waiting at the bound and the write completes on its own; see `src/policy/egress.rs`. |
