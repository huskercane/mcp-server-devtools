...
| `MCP_AUDIT_FORWARD_TOKEN` | — (required for HEC) | The HEC token (`Authorization: Splunk …`) or the bearer token for a JSON endpoint. May be a secret reference (`file://`, `vault://`). Never logged. |
