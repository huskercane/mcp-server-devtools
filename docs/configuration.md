> Edition boundary (September 8, 2026): console settings and
> `MCP_ADMIN_APPROVALS=required` require the Enterprise executable. Community
> keeps direct administration and refuses unsupported Enterprise settings.
...
| `MCP_OIDC_MACHINE_CLAIM` / `MCP_OIDC_HUMAN_CLAIM` | per profile | `oidc` mode only. A claim that marks a token as issued to a machine (a service account, a CI pipeline) or to a person, as `name` (present and not `null`) or `name=value` (that exact string, or `true`/`false`); `name` may be a dotted path like the groups claim. These **add to** the markers each profile already reads — Okta's `uid`/`cid`, Entra's `idtyp`, Auth0's `gty`, Keycloak's service-account mappers; the generic profile reads none — and any machine reading, the profile's or the operator's, outranks a human marker. A subject-name convention is never a marker. What the kind changes is `MCP_ADMIN_PRINCIPALS` below; see [service accounts](identity-provider-runbook.md#service-accounts-and-the-administrative-api). |
...
| `MCP_WRITE_MAX_TOKEN_AGE_SECONDS` | `300`; `0` disables | Plan §3.6 "high-risk operations": a non-read tool call is refused unless the token was issued (`iat`) within this many seconds — a token with no `iat` is refused for writes. Reads are unaffected. Applies only when inbound authentication is required. The same age bound also gates `/admin/proposals/{id}/approve`, including console approvals. |
| `MCP_CONSOLE_CLIENT_ID` | unset | Enterprise only; Community refuses this setting. Public OIDC PKCE client id; mounts `/console` only in a `console` feature build with OIDC authentication on `all`/`control`. No client secret. D.3 authoring and proposal decisions use the same authenticated session and configured approval gate; there is no separate console write bypass. See [console runbook](console-runbook.md). |
...
| `MCP_ADMIN_PRINCIPALS` | `human-or-unknown` | Which **kinds** of principal `/admin/*` admits, over and above `mcp:admin` (CF-42, AE-1). The kind is read from validated claims per profile (`MCP_OIDC_MACHINE_CLAIM` above). `human-or-unknown` refuses a token the provider positively marks as a machine's (`403 machine_principal`) and admits everything else, so a deployment whose tokens carry no marker is unchanged. `human` admits only tokens positively marked as a person's (`403 unverified_principal_kind` otherwise) — right where the profile marks both directions (Okta; Entra with `idtyp` on user tokens), and locks every administrator out, the console included, where it cannot. `any` admits machines and is refused at startup together with `MCP_ADMIN_APPROVALS=required`: the self-approval rule compares subjects, not kinds, so two service accounts would satisfy it with nobody in the loop. The MCP boundary is unaffected: a service account's tool calls are governed by policy, not by this. A profile that cannot see a machine (`generic` without a marker) logs a warning at startup. |
...
**Service accounts already authenticate.** A client-credentials (or RFC 7523
private-key JWT) access token from the configured issuer validates today
exactly as a person's does: issuer, audience, expiry, the subject claim and the
required scope, with groups empty when the token carries none. Nothing
requires a human-shaped claim, so a CI pipeline needs no setting here beyond
the policy that names it. The gateway does **not** advertise the draft
`io.modelcontextprotocol/oauth-client-credentials` extension, so a client that
requires advertisement before using the grant will not; that flag is proposed
in the [authorization extensions addendum](enterprise-auth-extensions.md).
What a service account may *administer* is `MCP_ADMIN_PRINCIPALS` above.
