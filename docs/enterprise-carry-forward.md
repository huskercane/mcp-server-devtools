...
Last updated: 2026-09-08, edition extraction and CF-40 build work completed locally.
The owner accepted the [edition boundary](edition-boundary-proposal.md).
The September 6 closeout below remains the prior evidence baseline.

Previously updated: 2026-09-06, D.0–D.4 closeout (rev 2.29).
...
(`transport::fetch_streamed_url`) originally bypassed this path. CF-15 below
records the explicit destination enforcement added in 0.17.0.
...
**Implemented · Community transport, integration expansion 0.17.0**

Shared API and streaming clients disable implicit redirects. Absolute downloads
carry originating call attribution, and each GET redirect hop is canonicalized,
authorized with its destination origin, deadline-bounded and journaled. Exact
origin allowlists gate signed downloads and cross-origin hops. API credentials
and custom headers never travel across origins. Loops, HTTPS downgrades and
non-GET redirects fail. Signed query values are omitted from download audit
records; all success/error body readers enforce actual byte limits.

`tests/integration_download_tests.rs` covers denied destinations with zero
requests, origin/path policy denial and redacted audit evidence, custom-header
stripping, relative redirects/loops, Retry-After deadlines, compressed error
limits, checksums and binary previews. Existing CircleCI fixtures now configure
trusted download origins. Existing upload paths remain in use.

See [implementation evidence and compatibility notes](integration-expansion-status.md)
and [download configuration](configuration.md#download-origins-bounds-and-binary-handling).
Credential bootstrap exceptions remain CF-17 (including Mend login/refresh).
DNS pinning and deferred SDK/browser transport enforcement are separate work.
No release tag is created by this implementation change.
...
**Done · September 8, 2026.**

Both editions now have `dev-fast` for optimized local iteration (opt-level 2,
LTO off, 16 codegen units, incremental, line tables). Shipping release and bench
profiles are unchanged. CI caches pinned dependencies/artifacts and the auditor;
default checks run together before headless checks. Enterprise uses the matching
core default feature flag, and its lockfile preserves the baseline dependency
versions. The remaining console-induced `winnow/simd` difference is documented.

The same-host/same-snapshot headless samples are 367.55 s release / 220.78 s
fast clean builds, 0.31 / 0.30 s no-op, and 232.17 / 5.99 s comment-only rebuilds.
The initial clean release sample overlapped an early check; the later comparison
was sequential. These are single local samples, not general code-edit or CI
speed guarantees. Cache conditions, memory measurements and commands are in
[build performance](build-performance.md) and its raw evidence.

Docker target caching also has a tested source-content guard: backdated changed
files force rebuilding the affected crate; failed builds never certify a new
fingerprint. All local edition gates pass, the shared allocation rows match,
and the console matches a fresh pre-extraction baseline. Static image and
DEB/RPM/archive checks passed. See [validation](edition-extraction-validation.md).
Hosted CI timing/signing and deployment portability remain their external-evidence
items; no required check was removed for speed.
...
before external enterprise contributions are accepted. The September 8 boundary
supersedes the all-current-code placement; prior ISC and Apache permissions are
preserved. Distribution and publication review remain open.
...
### CF-18 · Community/Enterprise implementation boundary (ADR-001)
**Implementation complete locally · September 8, 2026; publication review remains CF-10**

The extraction now builds and passes its local gates in both repositories; see
[validation and publication limits](edition-extraction-validation.md).

The owner accepted the [edition boundary](edition-boundary-proposal.md): keep the
secure gateway, existing administration API/CLI and operations Apache-2.0; extract
approvals and the browser console into Enterprise. Shared contracts remain in
Community. This supersedes both the September 5 all-current-code placement and
ADR-012's temporary co-location. Distribution/history review and commercial terms
remain CF-10; prior permissions are preserved. The earlier analysis below is
historical and does not override the September 8 boundary.
