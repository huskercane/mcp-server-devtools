...
## Current edition boundary — September 8, 2026

The owner accepted the [edition split](edition-boundary-proposal.md). Community
includes the secure gateway, OIDC/EMA, signed policy/revocation, audit, existing
secret sources/SIEM/rollups, and admin API/CLI. Enterprise contains approval
workflows and the browser console, with fleet operations, delegated credentials
and organizational reporting as future extensions. The earlier product hypothesis
below is retained as history; its feature packaging is superseded by this boundary.
