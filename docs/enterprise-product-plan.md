...
8. C.2 placement: the `SecretSource` port, cache, and `file://` step are in the community crate (rev 2.13, under the CF-18 reading). Still open for the three cloud adapters: community crate under Cargo features, or the enterprise crate? The packaging line in §6 lists secret providers as a tier differentiator, which argues for the enterprise crate; CF-18 argues for features while the repository is private. Decide before C.2b starts (CF-30).
