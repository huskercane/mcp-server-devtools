...
The September 8 [accepted edition boundary](edition-boundary-proposal.md)
supersedes earlier placement decisions: Community retains the secure gateway and
existing operations; approvals and console implementations live in Enterprise.
Historical phase/revision descriptions below describe where work originally landed.

## 0. Decision log

| Rev | Date | Change | Driven by |
|---|---|---|---|
| 2.31 | 2026-09-08 | Owner accepted and implemented the Community/Enterprise boundary: approvals, proposal CLI/endpoints and console extracted into the private Enterprise crate; shared enforcement and existing operations remain Apache-2.0. Separate executable/CI/image/packages validated locally. CF-40 adds measured dev-fast profiles, aligned feature/dependency caches and a tested Docker source-cache guard; shipping settings unchanged. [Validation](edition-extraction-validation.md) records tests, allocation comparisons, artifacts, history findings and external evidence still open. | Owner + implementation |
...
|---|---|---|---|---|
| ADR-001 | Repository and code boundary | Community is Apache-2.0 and independently buildable. Enterprise is a private proprietary crate depending one-way on the versioned Community library. Shared enforcement, ports, admin API/CLI and operations stay Community; approvals and browser console implementations are extracted. See [boundary](edition-boundary-proposal.md). | D — owner accepted 2026-09-08 | Separate binaries, CI and release artifacts; no Enterprise dependency in Community. |
| ADR-002 | Licence | Apache-2.0 Community; proprietary Enterprise extensions. Preserve permissions on previously supplied ISC/Apache copies. Agreement/contribution terms and distribution review remain CF-10; see [policy](licensing.md). | D — amended 2026-09-08 | Extraction changes future packaging, not prior grants. |
...
| ADR-012 | Phase D placement | Supersedes temporary co-location: D.2 approvals and D.1/D.3 console belong in Enterprise; shared D.0 contracts and D.4 packaging stay Community. D.5 delegated adapters are future Enterprise work, demand-gated. | D — owner accepted 2026-09-08 | Shared library composition replaces a duplicate gateway implementation. |
