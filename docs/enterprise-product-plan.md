...
8. C.2 placement: the `SecretSource` port, cache, `file://`, and now the `vault://` adapter (rev 2.15) are in the community crate under the CF-18 reading — Vault added no dependency, so the feature costs nothing and the question did not bite. Still open for the two cloud SDK adapters (C.2c, C.2d), where the SDK dependency trees are the real weight: community crate under Cargo features, or the enterprise crate? The packaging line in §6 lists secret providers as a tier differentiator, which argues for the enterprise crate; CF-18 argues for features while the repository is private. Decide before C.2c starts (CF-30).
